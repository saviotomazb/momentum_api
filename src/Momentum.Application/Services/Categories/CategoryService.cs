...
namespace Momentum.Application.Services.Categories
{
    public class CategoryService : ICategoryService
    {
        ...
        public async Task<CategoryResponse> CreateAsync(Guid userId, CreateCategoryRequest request)
        {
            ...
            {
                throw new InvalidOperationException($"Categoria com o nome '{request.Name}' já existe.");
            }

            var category = new Category(request.Name, request.Color, request.Icon, userId);
        ...
        public async Task<CategoryResponse?> GetByIdAsync(Guid userId, Guid categoryId)
        {
            ...
            {
                throw new UnauthorizedAccessException($"Acesso negado à categoria.");
            }
